...
static void GatherNestedObjects(UObject* Root, TArray<UObject*>& Out)
{
    Out.Reset();
    if (!Root)
    {
    ...
    if (Out.Num() > MaxNestedObjectsPerAsset)
    {
        Out.SetNum(MaxNestedObjectsPerAsset, EAllowShrinking::No);
    }
}

static UObject* FindNearestOuterByClassName(UObject* Object, const TCHAR* BaseClassName)
{
    for (UObject* Outer = Object ? Object->GetOuter() : nullptr; Outer; Outer = Outer->GetOuter())
    {
        if (ClassInheritsName(Outer->GetClass(), BaseClassName))
        {
            return Outer;
        }
    }
