...
class BlueprintInterproceduralCompositionTest(unittest.TestCase):
    def test_schema40_and_bundle_membership_are_composed_without_global_import_side_effects(self) -> None:
        facade = (SCRIPTS / "uatool.py").read_text(encoding="utf-8")
        self.assertIn("FINAL_DERIVED_SCHEMA_VERSION = 40", facade)
        ...
        self.assertIn("*blueprint_delegates.DERIVED_FILES", facade)
        self.assertIn("_require_blueprint_interprocedural(output)", facade)
        self.assertIn("_require_blueprint_delegates(output)", facade)
