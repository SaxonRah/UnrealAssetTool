#!/usr/bin/env python3
"""Promote accepted systems schema 9 and verify exact Dataflow / Geometry Collection graph topology."""
...
import uatool_dataflow_chaos_graph as dataflow_graph

ACCEPTANCE_MANIFEST = "systems_schema9_acceptance.json"
GRAPH_EXPECTATIONS_MANIFEST = "dataflow_chaos_graph_expectations.json"
GRAPH_VERIFICATION_MANIFEST = "dataflow_chaos_graph_verification.json"
TARGET_DERIVED_SCHEMA_VERSION = 25
LOSS_COUNT_KEYS = (
    "dataflow_chaos_truncated_properties",
    "dataflow_chaos_property_row_limit_hits",
)
REPRESENTATIVE_COUNT_KEYS = (
    ...
    "dataflow_pins",
    "dataflow_edges",
    "dataflow_node_properties",
    "geometry_collection_properties",
)
...
def _graph_expectations(root: Path, rows) -> dict:
    edges = dataflow_graph.expected_edge_keys(root, rows)
    relation_counts = collections.Counter(relation for _, relation, _ in edges)
    raw_files = tuple(dict.fromkeys(dataflow_graph.RELATION_STREAMS.values()))
    raw_counts = {name.removesuffix(".jsonl"): _count_rows(root, name, rows) for name in raw_files}
    return {
        "schema_version": 1,
        ...
        "expected_relation_counts": {
            ...
            for relation in sorted(dataflow_graph.RELATION_STREAMS)
        },
...
def _require_lossless_manifest(manifest: dict) -> dict:
    ...
    for key in LOSS_COUNT_KEYS:
        ...
        if value < 0:
            raise RuntimeError(f"systems schema 9 manifest is missing required loss counter: {key}")
        if value:
            raise RuntimeError(f"systems schema 9 Dataflow/Geometry Collection traversal is incomplete: {key}={value}")
    return counts


def _require_representative_content(counts: dict) -> None:
    ...
    if missing:
        ...
        raise RuntimeError(
            "systems schema 9 acceptance requires representative authored Dataflow + Geometry Collection content; " + details
        )


def accept_schema9(systems_module, project: Path, *, corpus: Path, systems_capture: Path) -> dict:
    if int(getattr(systems_module, "SYSTEMS_SCHEMA_VERSION", 0) or 0) != 9:
        raise RuntimeError("systems-schema9-accept requires composed systems schema 9")

    manifest = _read_json(systems_capture / "systems_manifest.json")
    if int(manifest.get("schema_version", 0) or 0) != 9 or not bool(manifest.get("success", False)):
        raise RuntimeError("isolated systems capture is not a successful schema-9 manifest")
    counts = _require_lossless_manifest(manifest)
    _require_representative_content(counts)
    error = systems_module.validation_error(systems_capture)
    if error:
        raise RuntimeError(f"isolated systems capture is not valid schema 9: {error}")

    stage = corpus / ".systems-schema9-accept-staging"
    if stage.exists():
        shutil.rmtree(stage)
    ...
    try:
        ...
        if error:
            raise RuntimeError(f"staged systems schema 9 failed validation: {error}")
        ...
        acceptance = {
            ...
            "project": str(project),
            "runtime_state_captured": False,
            "geometry_source_behavior_boundary": "excluded",
        ...
        for filename in systems_module.RAW_FILES:
            if filename == "systems_manifest.json":
                continue
            temp = corpus / f".{filename}.schema9-accept.tmp"
            shutil.copy2(stage / filename, temp)
            os.replace(temp, corpus / filename)
        manifest_temp = corpus / ".systems_manifest.json.schema9-accept.tmp"
        ...
        if error:
            raise RuntimeError(f"promoted canonical systems schema 9 failed validation: {error}")
        return acceptance
    finally:
        if stage.exists():
            shutil.rmtree(stage)
...
def _verify_graph(corpus: Path, rows) -> dict:
    ...
    if int(expectations.get("target_derived_schema_version", 0) or 0) != TARGET_DERIVED_SCHEMA_VERSION:
        raise RuntimeError("Dataflow/Chaos graph expectations target an unexpected derived schema")
    ...
    if actual_version != TARGET_DERIVED_SCHEMA_VERSION:
        raise RuntimeError(
            f"Dataflow/Chaos graph verification requires derived schema {TARGET_DERIVED_SCHEMA_VERSION}; got {actual_version}"
        )

    expected = dataflow_graph.expected_edge_keys(corpus, rows)
    domain_relations = set(dataflow_graph.RELATION_STREAMS)
    ...
    if actual != expected:
        ...
        raise RuntimeError("Dataflow/Chaos exact graph edge set mismatch: " + "; ".join(details))
    ...
    for row in actual_rows:
        ...
        if str(row.get("edge_quality", "")) != "exact_semantic":
            raise RuntimeError(f"Dataflow/Chaos relation is not exact_semantic: {relation}")
        expected_stream = dataflow_graph.RELATION_STREAMS[relation]
        evidence = row.get("evidence", []) if isinstance(row.get("evidence", []), list) else []
        if not any(isinstance(item, dict) and str(item.get("stream", "")) == expected_stream for item in evidence):
            raise RuntimeError(f"Dataflow/Chaos relation lacks canonical {expected_stream} evidence: {relation}")

    expected_counts = expectations.get("expected_relation_counts", {})
    for relation in dataflow_graph.RELATION_STREAMS:
        if int(expected_counts.get(relation, 0) or 0) != int(relation_counts.get(relation, 0)):
            raise RuntimeError(
                f"Dataflow/Chaos relation count mismatch for {relation}: expected={expected_counts.get(relation, 0)} "
    ...
    expected_nodes = {
        "dataflow": {
            ...
            for row in rows(corpus / "dataflow_graphs.jsonl") if row.get("asset_path")
        },
        "dataflow_node": {
            dataflow_graph.node_path(str(row.get("asset_path", "") or ""), str(row.get("node_guid", "") or ""))
            for row in rows(corpus / "dataflow_nodes.jsonl") if row.get("asset_path") and row.get("node_guid")
        },
        "dataflow_pin": {
            dataflow_graph.pin_path(str(row.get("asset_path", "") or ""), str(row.get("pin_guid", "") or ""))
            for row in rows(corpus / "dataflow_pins.jsonl") if row.get("asset_path") and row.get("pin_guid")
        },
        "geometry_collection": {
            str(row.get("asset_path", "") or "")
            for row in rows(corpus / "geometry_collections.jsonl") if row.get("asset_path")
        },
    }
    ...
    for kind, paths in expected_nodes.items():
        ...
        if missing:
            ...
            raise RuntimeError(f"Dataflow/Chaos graph nodes missing for {kind}: {len(missing)} first={first}")
...
def _accept_cli(systems_module, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="uatool systems-schema9-accept",
        description="promote an accepted systems-schema-9 capture into the canonical corpus; no Unreal or derive",
    )
    ...
    parser.add_argument("--systems-capture", help="capture directory; defaults to <corpus>/systems-schema9-capture")
    args = parser.parse_args(argv)
    project = _resolve_project(args.project)
    corpus = Path(args.corpus).expanduser().resolve() if args.corpus else project.parent / ".uatool"
    capture = Path(args.systems_capture).expanduser().resolve() if args.systems_capture else corpus / "systems-schema9-capture"
    result = accept_schema9(systems_module, project, corpus=corpus, systems_capture=capture)
    print(f"accepted canonical systems schema 9: {corpus}")
    ...
    print("  GeometrySource behavior boundary: excluded")
...
def _verify_cli(systems_module, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="uatool dataflow-chaos-graph-verify",
        description="verify derived-schema-25 Dataflow/Geometry Collection graph topology against accepted systems-schema-9 facts",
    )
    parser.add_argument("project", help="path to .uproject")
    parser.add_argument("--corpus", help="canonical corpus; defaults to <Project>/.uatool")
    args = parser.parse_args(argv)
    ...
    print(f"verified Dataflow/Geometry Collection derived-schema-25 project graph: {corpus}")
...
def install(runtime_module, systems_module) -> None:
    if getattr(systems_module, "__name__", "") != "uatool_systems":
        return
    if getattr(runtime_module, "_systems_schema9_accept_installed", False):
        return
    ...
    def main():
        if len(sys.argv) > 1 and sys.argv[1] == "systems-schema9-accept":
            try:
                return _accept_cli(systems_module, sys.argv[2:])
            except Exception as exc:
                print(f"ERROR: {exc}", file=sys.stderr)
                return 53
        if len(sys.argv) > 1 and sys.argv[1] == "dataflow-chaos-graph-verify":
            try:
    ...
    runtime_module._systems_schema9_accept_installed = True
