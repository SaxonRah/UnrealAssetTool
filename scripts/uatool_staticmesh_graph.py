#!/usr/bin/env python3
"""Promote exact mesh-schema-1 StaticMesh joins into derived schema 30."""
...
TARGET_DERIVED_SCHEMA_VERSION = 30
...
def _augment(output: Path, rows, nodes: list[dict], edges: list[dict], graph_module):
    ...
    def register(spec: dict):
        ...
        if node is None:
            node = {
                ...
                "family": str(spec.get("family", "static_mesh")),
            ...
            if str(node.get("family", "")) in ("", "asset_registry", "external"):
                node["family"] = "static_mesh"
        return node

    # Seed authoritative roots first so incoming references can never downgrade
    # exact class/package/root metadata.
    for row in rows(Path(output) / "static_meshes.jsonl"):
        register({
    ...
    for spec in data["edge_specs"]:
        ...
        source_node = node_by_key.get((sk, source)) or register({"path": source, "kind": sk, "coverage": "first_class", "family": "static_mesh"})
        target_node = node_by_key.get((tk, target)) or register({"path": target, "kind": tk, "coverage": "first_class", "family": "static_mesh"})
        ...
        if edge is None:
            edge = {
                ...
                "target_kind": tk,
                "target": target,
                "source_coverage": str(source_node.get("coverage", "first_class")),
                "target_coverage": str(target_node.get("coverage", "first_class")),
            ...
            edge["source_coverage"] = str(source_node.get("coverage", "first_class"))
            edge["target_coverage"] = str(target_node.get("coverage", "first_class"))
...
def install(project_graph_module, core_module=None, runtime_module=None) -> None:
    if getattr(project_graph_module, "_staticmesh_schema1_graph_installed", False):
    ...
    def derive(output, rows):
        ...
        if (Path(output) / "staticmesh_manifest.json").is_file():
    ...
    project_graph_module._staticmesh_schema1_graph_installed = True
    promote_public_derived_version(project_graph_module, core_module, runtime_module)
