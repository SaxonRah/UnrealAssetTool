#!/usr/bin/env python3
"""Promote exact Motion Warping joins into derived schema 32."""
...
def _augment(output: Path, rows, nodes: list[dict], edges: list[dict], graph_module):
    ...
    def register(spec: dict):
        ...
        if node is None:
            node = {
                ...
                "family": str(spec.get("family", "motion_warping")),
            ...
            if str(node.get("family", "")) in ("", "asset_registry", "external"):
                node["family"] = str(spec.get("family", "motion_warping"))
        return node
    ...
    for spec in data["edge_specs"]:
        ...
        if sm is None:
            sm = register({"path": source, "kind": sk, "coverage": "first_class", "family": "motion_warping"})
        if tm is None:
            tm = register({"path": target, "kind": tk, "coverage": "first_class", "family": "motion_warping"})
        if not sm or not tm:
            continue
...
def install(project_graph_module, core_module=None, runtime_module=None) -> None:
    if getattr(project_graph_module, "_motion_warping_graph_installed", False):
    ...
    project_graph_module._motion_warping_graph_installed = True
    promote_public_derived_version(project_graph_module, core_module, runtime_module)
