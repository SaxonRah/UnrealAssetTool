#!/usr/bin/env python3
"""Canonical UnrealAssetTool CLI with derived world and raw animation indexing."""
...
def _derive_world(output):
    ...
    asset_deps=list(_rows(output/"asset_dependencies.jsonl"))
    world_packages={x.get("package_name","") for x in worlds if x.get("package_name")}
    deps_by_source=collections.defaultdict(list)
    for dep in asset_deps:
        src=dep.get("source_package",""); dst=dep.get("target_package","")
        if src and dst: deps_by_source[src].append(dep)
    world_instance_classes={
        "/Script/Engine.LevelInstance":"level_instance",
        "/Script/Engine.PackedLevelActor":"packed_level_actor",
    }
    for x in worlds:
    ...
    for x in descs:
        ...
        if instance_kind:
            ...
            for dep in deps_by_source.get(x.get("actor_package",""),[]):
                target=dep.get("target_package","")
                if target in world_packages and target!=owner_package:
                    category=dep.get("category","")
                    if category: candidates[target].add(str(category))
                    else: candidates[target]
            if len(candidates)==1:
                target=next(iter(sorted(candidates)))
...
def create_schema(c):
    _old_schema(c); c.executescript(_SQL); world_stitch.create_schema(c); animation.create_schema(c)
...
def build_database(output):
    ...
    try:
        ...
        world_stitch.load_database(c,output,_rows); animation.load_database(c,output,_rows); c.commit()
    finally:c.close()
    return db
def query(a):
    ...
    try:
        ...
        animation.query(c,core._print_rows,q,a.limit)
    finally:c.close()
...
def _summary(a):
    ...
    print("animation scan complete: "+line(ac,("animation_assets","animation_notifies","animation_sync_markers","skeletons","skeleton_bones","pose_search_databases","pose_search_database_assets","pose_search_schemas","pose_search_channels","animation_optional_assets")))
...
def scan(a):
    ...
    if r==0:
        error=animation.validation_error(_output(a))
        if error:
...
core.DEFAULT_BUNDLE_FILES=tuple(dict.fromkeys((*core.DEFAULT_BUNDLE_FILES,*WORLD_DERIVED_FILES,*animation.RAW_FILES)))
